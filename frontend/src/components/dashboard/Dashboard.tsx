...
export default function DashboardClient({ category }: DashboardClientProps) {
  ...
  const initializeDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (inputs.prediction_id) {
        await loadSinglePredictionData(parseInt(inputs.prediction_id));
  ...
  }, [category, setInputs, loadSinglePredictionData]);
  ...
  const togglePrediction = useCallback(
    async (prediction: Prediction) => {
      ...
      if (isSelected) {
        ...
        await loadSinglePredictionData(prediction.id);
      }
    },
    [chartPredictions, loadSinglePredictionData]
  );
  ...
  const toggleInterval = (predictionId: number) =>
    setActiveIntervals(prev => {
      const next = new Set(prev);
      next.has(predictionId) ? next.delete(predictionId) : next.add(predictionId);
      return next;
    });
  ...
  const handleSelectAll = async () => {
    const unselected = filteredAndSortedPredictions
      .filter(p => !chartPredictions.some(cp => cp.id === p.id))
      .slice(0, 10);

    if (unselected.length === 0) return;

    setLoadingPredictions(prev => [...prev, ...unselected.map(p => p.id)]);
  ...
  const uniqueModels = useMemo(() => {
    const baseList =
      selectedSprints.length > 0
        ? predictions.filter(p => p.sprint && selectedSprints.includes(p.sprint))
        : predictions;

    return Array.from(new Set(baseList.map(p => p.repository))).sort();
  ...
  const filteredAndSortedPredictions = useMemo(() => {
    ...
    if (selectedSprints.length > 0) {
      result = result.filter(
        p => p.sprint && selectedSprints.includes(p.sprint)
      );
    }

    if (!inputs.sprint) {
      result = result.filter(
        p => p.case_definition === inputs.case_definition
      );
    }

    if (selectedModels.length > 0) {
      result = result.filter(p =>
        selectedModels.includes(p.repository)
      );
    }
    ...
    return [...result].sort((a, b) => {
      const sA = selectedIds.has(a.id);
      const sB = selectedIds.has(b.id);
      ...
      if (sortConfig.key) {
        const scA =
          a.scores.find(s => s.name === sortConfig.key)?.score ?? null;
        const scB =
          b.scores.find(s => s.name === sortConfig.key)?.score ?? null;
  ...
  return (
    <div className="p-4 relative space-y-6 min-h-[500px] max-w-full">
      ...
      <DashboardParameters
        ...
        handleCaseDefinitionChange={handleCaseDefinitionChange}
      ...
      <DashboardPredictions
        ...
        paginatedPredictions={filteredAndSortedPredictions.slice(
        ...
        totalPages={Math.ceil(
          filteredAndSortedPredictions.length / ITEMS_PER_PAGE
        )}
