...
describe("app/api/datastore/cities", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("returns data and forwards query params", async () => {
    ...
    const { GET } = await import("./route");
    const res = await GET(makeReq("fs", { uf: "SP", limit: "10" }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual([{ city: "A" }]);
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain("/api/datastore/cities/");
