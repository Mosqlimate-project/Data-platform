...
describe("app/api/datastore/charts/contaovos/positivity", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("forwards body with query params", async () => {
    ...
    const { GET } = await import("./route");
    const res = await GET(makeReq("fs", { uf: "SP", start: "1", end: "2" }));
    expect(res.status).toBe(200);
    expect(await res.text()).toBe("data");
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain("positivity/?uf=SP&start=1&end=2");
  });
  ...
  it("omits absent params", async () => {
    ...
    expect(res.status).toBe(200);
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain("positivity/?start=1");
