...
describe("app/api/datastore/charts/contaovos/eggs-density", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("forwards body with all query params", async () => {
    (global.fetch as any).mockResolvedValue(okRes("data", 200, "application/json"));
    const { GET } = await import("./route");
    const res = await GET(
      makeReq("fs", { uf: "SP", geocode: "35", start: "1", end: "2" })
    );
    expect(res.status).toBe(200);
    expect(await res.text()).toBe("data");
    expect(res.headers.get("Content-Type")).toBe("application/json");
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain("eggs_density/?uf=SP&geocode=35&start=1&end=2");
  });
  ...
  it("omits absent query params", async () => {
    ...
    expect(res.status).toBe(200);
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain("eggs_density/?start=1");
