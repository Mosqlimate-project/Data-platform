...
describe("app/api/datastore/charts/infodengue/total-cases", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("forwards body with query params", async () => {
    ...
    const { GET } = await import("./route");
    const res = await GET(
      makeReq("fs", { disease: "dengue", geocode: "35", start: "1", end: "2" })
    );
    expect(res.status).toBe(200);
    expect(await res.text()).toBe("data");
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain(
      "infodengue/total-cases/?disease=dengue&geocode=35&start=1&end=2"
    );
  });
  ...
  it("forwards upstream error status", async () => {
    ...
    const { GET } = await import("./route");
    const res = await GET(makeReq("fs", { disease: "dengue", geocode: "35" }));
    expect(res.status).toBe(500);
