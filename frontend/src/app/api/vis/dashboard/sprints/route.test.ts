...
const makeReq = (secret?: string, cookies: Record<string, string> = {}, params: Record<string, string> = {}) =>
  ({
    headers: {
...
describe("app/api/vis/dashboard/sprints", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("returns data with query params and auth header", async () => {
    ...
    const { GET } = await import("./route");
    const res = await GET(
      makeReq("fs", { access_token: "at" }, { category: "cat", disease: "dengue", skip: "x" })
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual([{ sprint: 1 }]);
    const [url, init] = (global.fetch as any).mock.calls[0];
    expect(String(url)).toContain("/api/vis/dashboard/sprints/?category=cat&disease=dengue");
  ...
  it("maps upstream error status", async () => {
    ...
    expect(await res.json()).toEqual({ error: "Failed to fetch sprints" });
  });
