...
const params = { prediction_id: "7" };

describe("app/api/vis/dashboard/prediction/[prediction_id]/metadata", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("returns data on success", async () => {
    (global.fetch as any).mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({ metadata: 1 }),
    });
    const { GET } = await import("./route");
    const res = await GET(makeReq("fs"), { params: params as any });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ metadata: 1 });
    const url = String((global.fetch as any).mock.calls[0][0]);
    expect(url).toContain("/api/vis/dashboard/prediction/7/metadata/");
  });
  ...
  it("maps upstream error status", async () => {
    ...
    expect(await res.json()).toEqual({ error: "Failed to fetch prediction data" });
  });
