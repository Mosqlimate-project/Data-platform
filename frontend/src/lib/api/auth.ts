import { NEXT_PUBLIC_BACKEND_URL, BACKEND_BASE_URL } from "@/lib/env";


export function oauthLogin(provider: "google" | "github" | "gitlab", next?: string) {
  window.location.href = `${NEXT_PUBLIC_BACKEND_URL}/api/user/oauth/login/${provider}/?next=${encodeURIComponent(next || "")}`;
}
