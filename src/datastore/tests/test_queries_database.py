...
from datastore.vis_charts import get_data, uf_ibge_mapping
import unittest
...
class EchartsVisDataTestCase(TestCase):
    ...
    @unittest.skip("Database migrations didn't find the 'historico_alerta' table")
    def test_get_data(self):
        ...
        for result in response:
            ...
            self.assertIn(
                result["name"], [data["name"] for data in uf_ibge_mapping.values()]
            )

        entry_3304557 = next(
            (result for result in response if result["name"] == "Rio de Janeiro"), None
        )
        self.assertIsNotNone(entry_3304557)
        self.assertEqual(entry_3304557["value"], 68253)

        self.assertIn("Rio de Janeiro", [result["name"] for result in response])
